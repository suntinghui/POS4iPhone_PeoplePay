...
- (void)sizeToFitOrientation:(BOOL)transform
{
    ...
    CGFloat scaleFactor = SinaWeiboIsDeviceIPad() ? 0.8f : 1.0f;
...
- (void)showWebView
{
    ...
    modalBackgroundView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
...
- (BOOL)webView:(UIWebView *)aWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([urlStr hasPrefix:appRedirectURI] || [urlStr hasPrefix:siteRedirectURI])
    {
        ...
        {
            NSString *code = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"code"];
            if (code)
            {
                [self hide];
