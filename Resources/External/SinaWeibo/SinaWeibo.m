...
//            }
        ...
        if (!ssoLoggingIn)
        {
            ...
            [[SinaWeiboAuthorizeView alloc] initWithAuthParams:params
                                                      delegate:self];
            [authorizeView setFrame:CGRectMake(0, 0, 900, 700)];
            [authorizeView show];
        }
    }
}
...
- (BOOL)handleOpenURL:(NSURL *)url
{
    ...
    if ([urlStr hasPrefix:self.ssoCallbackScheme])
    {
        if (!ssoLoggingIn)
        {
        ...
        else
        {
            ...
            if ([SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"sso_error_user_cancelled"])
            {
                if ([delegate respondsToSelector:@selector(sinaweiboLogInDidCancel:)])
                {
                    [delegate sinaweiboLogInDidCancel:self];
            ...
            else if ([SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"sso_error_invalid_params"])
            {
                if ([delegate respondsToSelector:@selector(sinaweibo:logInDidFailWithError:)])
                {
            ...
            else if ([SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"error_code"])
            {
                NSString *error_code = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"error_code"];
                NSString *error = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"error"];
                NSString *error_uri = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"error_uri"];
                NSString *error_description = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"error_description"];
            ...
            {
                NSString *access_token = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"access_token"];
                NSString *expires_in = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"expires_in"];
                NSString *remind_in = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"remind_in"];
                NSString *uid = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"uid"];
                NSString *refresh_token = [SinaWeiboRequest getParamValueFromUrl:urlStr paramName:@"refresh_token"];
